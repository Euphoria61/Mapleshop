...
import com.euphoria.shop.security.filter.MobileCodeLoginToken;
...
public class UserServiceImp extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    @Override
    public ResultInfo<?> login(User user, int type, HttpServletRequest request) {
        //authenticate进行用户认证
        ...
        if (type == 0) {
            ...
            authenticationToken = new UsernamePasswordAuthenticationToken(user.getUserName(), user.getUserPwd());
            authenticate = authenticationManager.authenticate(authenticationToken);
        }
        //todo 第三方短信 --》接受code
        if (type == 1) {
            authenticationToken = new MobileCodeLoginToken(user.getUserName(), user.getUserPwd());
            authenticate = authenticationManager.authenticate(authenticationToken);
        }
        //如果认证未通过
